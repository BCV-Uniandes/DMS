...
def convert_from_mxnet(model, checkpoint_prefix, debug=False):
    _, mxnet_weights, mxnet_aux = mxnet.model.load_checkpoint(
        checkpoint_prefix, 0)
    remapped_state = {}
    for state_key in model.state_dict().keys():
        ...
        if k[0] == 'features':
            if k[1] == 'conv1_1':
                ...
                if model.b and 'c1x1_c' in k[2]:
                    # b-variant split c-block special treatment
                    bc_block = True
                else:
                    bc_block = False
                ck = k[1].split('_')
        ...
        if debug:
            print(list(
                torch_tensor.size()), torch_tensor.mean(), torch_tensor.std())
...
parser = argparse.ArgumentParser(
    description='MXNet to PyTorch DPN conversion')
parser.add_argument(
    'checkpoint_path', metavar='DIR', help='path to mxnet checkpoints')
...
def main():
    ...
    model = model_factory.create_model(args.model, num_classes=1000,
                                       pretrained=False)
    ...
    output_checkpoint = os.path.join(
        args.checkpoint_path, model_prefix + '.pth')
