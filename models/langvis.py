...
class LangVisNet(nn.Module):
    def __init__(self, dict_size, emb_size=1000, hid_size=1000,
                 ...
                 extra=True, gpu_pair=None, high_res=False, concat_rt=False):
        super().__init__()
        ...
        if self.concat_rt:
            rt_size = emb_size + hid_size
        else:
            rt_size = 0
        
        self.comb_conv = nn.Conv2d(in_channels=(8 + rt_size +
    ...
    def forward(self, vis, lang):
        ...
        else: # Do not use lang_mix
            q = torch.cat([vis, p], dim=2)
        # Lx1xSxH/32xW/32
        # print(mixed.size())
