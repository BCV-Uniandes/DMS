...
"""
DeepLab-ResNet Network PyTorch implementation.

Liang-Chieh Chen, George Papandreou, Iasonas Kokkinos,
Kevin Murphy, Alan L. Yuille:

DeepLab: Semantic Image Segmentation with Deep Convolutional Nets,
Atrous Convolution, and Fully Connected CRFs. CoRR abs/1606.00915 (2016)
https://arxiv.org/pdf/1606.00915

Taken from:
https://github.com/speedinghzl/Pytorch-Deeplab/blob/master/deeplab/model.py
"""
...
class Bottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1,
        ...
        self.conv2 = nn.Conv2d(planes, planes, kernel_size=3,
                               ...
                               bias=False, dilation=dilation)  # change
        self.bn2 = nn.BatchNorm2d(planes, affine=affine_par)
    ...
    def __init__(self, dilation_series, padding_series, num_classes):
        ...
        for dilation, padding in zip(dilation_series, padding_series):
            self.conv2d_list.append(nn.Conv2d(
                2048, num_classes, kernel_size=3, stride=1,
...
class ResNet(nn.Module):
    def __init__(self, block, layers, num_classes):
        ...
        self.layer5 = self._make_pred_layer(
            ClassifierModule, [6, 12, 18, 24], [6, 12, 18, 24], num_classes)
    ...
    def _make_pred_layer(
            self, block, dilation_series, padding_series, num_classes):
...
class MSDeepLab(nn.Module):
    def __init__(self, block, num_classes):
        super(MSDeepLab, self).__init__()
    ...
    def forward(self, x):
        ...
        self.interp1 = nn.Upsample(
            size=(
                ...
                int(input_size * 0.75) + 1), mode='bilinear')
        self.interp2 = nn.Upsample(
            size=(
                int(input_size * 0.5) + 1,
                int(input_size * 0.5) + 1), mode='bilinear')
...
def ResMsDeeplab(num_classes=21):
    model = MSDeepLab(Bottleneck, num_classes)
    return model


def ResDeeplab(num_classes=21):
